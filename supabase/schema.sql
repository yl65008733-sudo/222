...
drop policy if exists "Anyone can read guestbook photos" on storage.objects;
create policy "Anyone can read guestbook photos"
on storage.objects
for select
using (bucket_id = 'guestbook-photos');

drop policy if exists "Anyone can add guestbook photos" on storage.objects;
create policy "Anyone can add guestbook photos"
on storage.objects
for insert
